...
// directory (e.g. "ja/my-post"). Routing uses /posts/<slug>/ (ja, default)
// and /en/posts/<slug>/ (en) without the lang segment, so strip it here.
...
export function getPostHref(entry: CollectionEntry<'posts'>): string {
  ...
  return entry.data.lang === 'en' ? `/en/posts/${slug}/` : `/posts/${slug}/`;
}
